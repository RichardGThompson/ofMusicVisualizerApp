...
{
    m_font.load( "fonts/font.ttf", 10, true );
...
void AudioAnalyser::drawWaveform( int _x, int _y, int _width, int _height )
{
	...
	ofPushMatrix();
    	...
    	ofFill();
        m_font.drawString( "Waveform", 10, 25 );
...
void AudioAnalyser::drawSpectrum( int _x, int _y, int _width, int _height )
{
	...
	ofPushMatrix();
        ...
        ofSetColor( ofColor::white );
        m_font.drawString( "FFT Spectrum", 10, 25 );
    ...
    if ( mouseY > _y && mouseY < _y + _height )
    {
        ...
        if ( mouseX > _x && mouseX < _x + _width )
        {
            ...
            m_font.drawString( "(" + ofToString( index ) + ") Freq: " + ofToString(index * freqStep) + " to " + ofToString((index+1) * freqStep) + ",\nValue: " + ofToString( val * 1000.0f ), mouseX + 8, mouseY - 30 );
        }
    }
}

void AudioAnalyser::styleSpectrum(int _x, int _y, int _width, int _height) {
    int binWidth = (float)_width / (float)((m_numSpectrumBins - 1) / 2);
    float halfHeight = _height * 0.5f;
...
void AudioAnalyser::drawLinearAverages( int _x, int _y, int _width, int _height )
{
	...
	ofPushMatrix();
    	...
    	for (int i = 0; i < m_numLinearAverages; i++) 
        {
            ...
            ofSetColor( ofColor::white );
            m_font.drawString( ofToString( i ), i * binWidth + binWidth/2.0f - 7, 30 );
    	}
        
        ofSetColor( ofColor::white );
        m_font.drawString( "Linear Averages", 10, 60 );
    ...
    if ( mouseY > _y && mouseY < _y + _height )
    {
        ...
        if ( mouseX > _x && mouseX < _x + _width )
        {
            ...
            ofDrawLine( _x, mouseY, _x + _width, mouseY ); 
            m_font.drawString( "(" + ofToString( index ) + "): " + ofToString( val * 1000.0f ), mouseX + 8, mouseY - 10 );
        }
    }
}
